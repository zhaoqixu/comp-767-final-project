...
class StockTrader():
    ...
    def print_result(self,epoch,agent,noise_flag):
        ...
        agent.write_summary(self.loss, self.total_reward, self.ep_ave_max_q, self.actor_loss)
...
def session(config,args):
    ...
    if args['mode']=='train':
        ...
        backtest([PPO(predictor, len(codes) + 1, int(window_length), len(features), '-'.join(agent_config), reload_flag,
            trainable)],env)



def build_parser():
    parser = ArgumentParser(description='Provide arguments for training different PPO models in Portfolio Management')
