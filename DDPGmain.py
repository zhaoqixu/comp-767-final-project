...
class StockTrader():
    ...
    def print_result(self,epoch,agent,noise_flag):
        ...
        self.criloss.append(self.loss)
        self.actloss.append(self.actor_loss)
...
def session(config,args):
    ...
    if args['mode']=='train':
        if not os.path.exists(PATH_prefix):
            ...
            with open("result/DDPG/" + str(args['num']) + '/config.json', 'r') as f:
        ...
        backtest([DDPG(predictor, len(codes) + 1, int(window_length), len(features), '-'.join(agent_config), "True",
                     "False")],env)
def build_parser():
    parser = ArgumentParser(description='Provide arguments for training different DDPG models in Portfolio Management')
