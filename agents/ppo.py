...
class PPO(object):
    ...
    def __init__(self,predictor,M,L,N,name,load_weights,trainable):
        ...
        with tf.variable_scope('critic'):
            ...
            C_learning_rate = tf.train.exponential_decay(C_LR, self.global_step,
                                                       decay_steps=2000,
                                                       decay_rate=0.9, staircase=False)
            self.ctrain_op = tf.train.GradientDescentOptimizer(C_learning_rate).minimize(self.closs,global_step=self.global_step)
        ...
        with tf.variable_scope('atrain'):
            A_learning_rate = tf.train.exponential_decay(A_LR, self.global_step,
                                                         decay_steps=2000,
                                                         decay_rate=0.9, staircase=False)
    ...
    def write_summary(self, Loss, reward, ep_ave_max_q, actor_loss):
    ...
    def save_model(self):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        self.saver.save(self.sess, path + self.name, global_step=self.global_step)
