...
class PG:
    def __init__(self,M,L,N,name,load_weights,trainable,type,number):
        ...
        if trainable == 'True':
            # Initial summary
            self.summary_writer = tf.summary.FileWriter('./summary/PG'+self.number+'/'+'summary/'+type+'/', self.sesson.graph)
    ...
    def train(self):
        ...
        loss,profit,_=self.sesson.run([self.loss,self.profit,self.optimize],feed_dict={self.state:s,self.out:np.reshape(a,(-1,self.M)),self.future_price:np.reshape(p,(-1,self.M)),self.w_previous:np.reshape(a_previous,(-1,self.M))})
        return loss
    ...
    def write_summary(self,Loss,reward):
    ...
    def build_summaries(self):
        ...
        self.Lo = tf.Variable(0.)
        tf.summary.scalar('Reward', self.reward)
        tf.summary.scalar('Loss', self.Lo)
        summary_vars = [self.Lo,self.reward]
